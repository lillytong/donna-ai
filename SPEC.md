...
All design decision records (DD-01 … DD-80) live in **[`DESIGN_DECISIONS.md`](DESIGN_DECISIONS.md)** — see its index for the full list. Inline `DD-NN` references throughout this spec resolve there.
...
8. **Counterparty change review UI** — by now (6b done) every change is classified; the three reviewable types (matched-with-edits / new / deleted) flow through **one linear stream in document order** (DD-78), not three bucket-screens — top-to-bottom mirrors how the operator reads the contract and preserves the narrative of what the counterparty did (a deletion at clause 4 sitting next to an edit at clause 5). Left panel is the changes navigation list (numbered, ✓ reviewed / ● current / blank = pending). Right panel shows a **type-appropriate card**, all on the DD-26 inline-tracked-markup + DD-27 four-action pattern:
   - **Matched-with-edits** — counterparty edit as inline strikethrough/underline, Donna's counter below: **Accept theirs** / **Use Donna's counter** / **Edit Donna's counter** / **Keep original**.
   - **New (counterparty added)** — the added clause rendered as an insertion, Donna's significance read + optional counter: **Accept addition** / **Use Donna's counter** / **Edit** / **Reject** (rejecting reinstates our version → tracked deletion in the next redline, and the rejection seeds an issue per step 9).
   - **Deleted (counterparty removed)** — the removed clause rendered as a deletion, Donna's read: **Accept deletion** / **Reject** (reinstate → tracked insertion) / **Edit** (reinstate a modified form). A *new* node already carries its position in the incoming tree, so there is no separate "placement" action — the decision is purely accept/reject the counterparty's move.
