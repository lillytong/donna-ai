...
| F03b | Counterparty revision import (incoming .docx → change list) | P0 | 2 | **Path B backend done** (Path A + Donna-significance deferred) | Triggered from cockpit; diffs against last snapshot; two parse paths (tracked changes vs. clean diff); see §11. **BUILT (Path B clean-diff, backend):** `POST /contracts/{id}/revisions/import?source=counterparty\|legal` (raw `.docx` body — matched Mode-A's no-multipart convention, `python-multipart` not in stack) → detect tracked markup (→422, Path A deferred) → parse incoming → adapt to `ClauseNode` → wire `match_revision` (unmodified, de-risked: real-pair precision 1.000) against the `last_shared_with_{party}` snapshot → persist `as_received` snapshot (JSONB) + `received` pointer (v-number = snapshot order, DD-70) + `counterparty_revision_session/_changes/_hunks`. Buckets: edited-match → change(node_id=baseline)+difflib word-hunks; new → change(node_id NULL, order_index set)+whole-body insertion hunk; deleted → change(node_id=baseline)+whole-body deletion hunk; abstain → change(node_id NULL, order_index NULL, `match_confidence`)+provisional hunks. Single-open-session guard (409); no-baseline 409; DD-63 cascade extended (FK-correct: revision tables after `issues`, before nodes/snapshots). New schema: `counterparty_revision_changes.match_confidence` (migration `0008`, additive). Hunk significance hard-defaulted `substantive`. **Gate:** 527 tests / mypy clean (99 files) / ruff clean (17 new). **Deferred:** Path-A tracked-changes extraction (422 stub), Donna LLM hunk-significance. **Classification inheritance now LIVE** (DD-82, with F03c writing live `nodes`): a matched revised node inherits its baseline role, only genuinely-new nodes default to `clause` — no re-classification. DD-78 new/deleted whole-node decision = single-hunk change, no schema change needed |
| F03c | Counterparty change review — accept/reject/modify + Donna counter-language | P0 | 2 | **built** (backend + frontend; real-data apply-spine verified; pending Lilly UX acceptance) | Two-pane document view (DD-81): single inline embedded full-clause redline (insertions green/underline, deletions red strikethrough, read in place like Word); **per-change (Word-style) decisions** — each edit its own Accept theirs/Use Donna's/Edit/Keep; Donna's counter **auto-runs at import** (DD-82) so it's populated on open; revised side **inherits baseline classification** (DD-28/DD-54). DD-26, DD-27. **BUILT (review + decision API, backend, DD-78/DD-79):** `services/import_/revision_review.py` + `api/revision_review.py` (6 thin routes) + `models/revision_review.py`. `GET …/revisions/sessions` (reviewing-first) · `GET /revisions/sessions/{id}` → `ReviewPayload{session, phase1{abstains ranked asc confidence, tree_anomalies:[]}, phase2 (doc-ordered)}` · `POST …/changes/{id}/confirm-match {confirm\|new\|rematch}` (6b) · `POST …/hunks/{id}/decide {accept\|counter\|edit\|keep}` (DD-27) · `POST …/changes/{id}/decide-node {accept\|reject\|edit}` (new/deleted whole-node) · `POST …/sessions/{id}/apply`. **`change_kind` derived from F03b's staged columns — NO migration (DD-79):** edited=node_id+confidence · deleted=node_id+NULL confidence · new=NULL node_id+order_index · abstain=both NULL; the existing hunk `verdict` CHECK + `final_text` carry decisions. **Apply (one txn):** reuses F08 edit/insert/delete paths, reconstructs edited body from baseline + each hunk's final_text (descending offsets), seeds an `issue` (`initiator=counterparty`, `category=counterparty_proposed_edit`, `counterparty_revision_session_id`) per rejected hunk/node → session `completed` + `revision_session_applied` audit. **Gate:** 551 passed / 1 skipped, mypy strict + ruff clean (`cf4b37e`). **POST-UX-FEEDBACK builds (Lilly live-test, 2026-06-25):** (a) **clause context on EVERY card** (`5a6e17c`) — number + heading + ancestor breadcrumb on each card; edits rendered IN PLACE within the full surrounding clause body (not isolated fragments); new/deleted show landing neighbours; abstains both-sides. Read-only enrichment of `get_review_payload`, exact `node_id` resolution for edited/deleted, no schema change. Live-verified. (b) **Donna per-change recommendation engine** (`f8b6ea9`) — `POST /revisions/sessions/{id}/recommend` runs one Opus call/hunk → writes `donna_verdict`(accept\|counter\|keep) + `significance` + exact `donna_counter_text` onto the advisory columns (NEVER `final_text` — DD-64: redline stays the deterministic diff, counter is operator-adopted via "Use Donna's"). Reuses F11 grounding + logging LLM wrapper; prompt wraps document text as data-not-instructions (injection defense, prompt-level only — see DEV_TODO follow-up). Live-verified grounded counters on the real licensing-agreement test pair (a numeric middle-ground on one term, a revert-to-baseline on another). **(c) Resume affordance + open-session-aware badge** (2026-06-26, Kevin-verified) — cockpit detects an open `reviewing` session → persistent Resume strip routing to `/contracts/{id}/revisions/{sessionId}` (resolves the back-out wedge: operator could neither resume nor re-import past the 409 single-open guard, which stays as-is per DD-48); lifecycle badge gains `Reviewing revision · vN` (new Rule 1b in `lineage._compose_badge`, one additive `EXISTS` subquery in `_LIST_BADGES` — preserves the set-based no-N+1 invariant) so home + My-Contracts + cockpit all stop reading "Your move" while a review is open; `StoredRevisionSession.pending_changes` (count of changes `status<>'complete'`) surfaced via correlated subquery (no N+1). No schema change — pure surfacing of already-persisted state. **Gate:** 592 passed / 1 skipped, mypy strict clean (105 files), ruff + tsc clean. **(d) Two-pane document review rework** (2026-06-26, Kevin-verified at compile; visual pending Lilly's UX acceptance — see DD-81) — review surface changed from a card-stream to a two-pane document view. New read-only endpoint `GET …/revisions/sessions/{id}/document` → `RevisionDocumentView{baseline[], revised[], changes[](kinds: added\|deleted\|modified), abstain_matches[]}` (both trees via the `get_snapshot_tree` adapter; baseline roles recovered by batched join to live `nodes`; revised synthetic ids + new clauses default `clause`; "shifted" kind DESCOPED — not backable by staged data). **Left rail = navigator + to-do tracker:** one row per change, anchor label = clause number else role-based fallback ("Appendix title"/"Draft note"/"Recital"/"New clause"), Added/Deleted/Modified tags, decided-state tick/strike, prominent "N pending". **Phase 1 = before/after:** baseline │ revised columns, each abstain pair highlighted in BOTH sides; controls reuse `confirm-match`; bulk confirm/new = client-side loop (no server bulk endpoint). **Phase 2 = content:** document in reading order, changed clauses NEUTRAL-highlighted (kind colour only on the rail tags), click → inline expand (text-push) → redline + Donna verdict + `donna_counter_text` + Accept theirs/Use Donna's/Edit/Keep (`decide`) and whole-node Accept/Use Donna's/Reject/Edit (`decide-node`, "Use Donna's" added → closes the whole-node Edit-only Open item). **Brainstorm escalation** reuses the ephemeral overlay skin + `brainstorm` endpoints, seeded per-change. Perf: flat memoized `DocRow` list, local-state inline editor, O(changed) interactions, no virtualization dep. **Gate:** tsc 0, both CSS-module imports resolve on disk, live `next dev` recompile HTTP 200. **(e) Inline full-clause redline + per-change decisions + Donna auto-run-at-import** (2026-06-26, commit `8897799`, operator-verified live — RESOLVES DD-81). Phase-2 rendering changed from three redundant representations (baseline / revised / isolated per-hunk diff) to a **single inline embedded redline of the FULL clause** — complete clause text read in place like Word, **insertions green/underline + deletions red strikethrough** (neither the prior baseline-spine nor a revised-text default; DD-81 open deviation resolved). **Decision granularity is per-change (Word-style, DD-81):** a clause with multiple edits exposes each as its own "Change N" row with its own Accept theirs/Use Donna's/Edit/Keep — never one decision for the whole clause (whole-node added/deleted stay one node decision). **Donna's per-change recommendation now auto-runs in the background at import (DD-82):** verdict + counter-language are populated when the review opens (failure-isolated, cost-guarded, substantive-only), so "Use Donna's" is live on open — previously never ran in this flow. **Classification inheritance confirmed live (DD-28/DD-54/DD-82):** the revised side inherits each matched node's baseline role; only genuinely-new nodes default to `clause`. **Frontend follow-ups:** brainstorm distill-on-close is a no-op for undecided changes (issues seeded only at apply → no brainstorm target; nil-UUID sentinel degrades gracefully); added-clause insertion point approximated to after `proposed_parent_id`; bulk match-confirm is a client loop. **Deferred:** 6a tree-shape anomaly source (returns []), verdict-label mapping for new/deleted cards (Donna's keep/accept vs the card's Reject/Accept-addition — cosmetic), F03d rich logging. **Two phases (DD-78):** (1) **structural foundation** — §11 step 6a tree-shape triage + **6b abstain match-confirm** (the Mode-B matcher's abstain bucket, DD-64: ranked queue, per-pair [Confirm match]/[Not a match → new]/[Match to different clause], bulk "confirm-remaining"/"rest-are-new"; resolving each reclassifies it into matched/new/deleted so content review starts fully classified); (2) **content review** — one **document-ordered linear stream** of the three settled types with type-appropriate cards (matched-with-edits: Accept theirs/Use Donna's/Edit/Keep · new: Accept addition/Use Donna's/Edit/Reject · deleted: Accept deletion/Reject/Edit), not four bucket-screens. **Load discipline:** no blocking cap; matcher abstention gate (≤~15% of changed nodes) is engineering's quality metric, the ranked+bulk queue is the product guard; rewrite-scale diff → non-blocking "re-import as Mode A baseline" escape hatch (§11 step 4). See §11 Mode B |
...
| F10b | Donna chat — context-aware advise + draft (Brainstorm hand-off) | P1 | 2 | **built** | Extends the F10 Donna tab beyond read-and-explain: when an active context is set (selected clause and/or open issue, shown as a context chip), Donna advises (F11 engine) + drafts/revises (F08d engine) grounded+cited to that anchor; with no context she stays F10 read-and-explain and deflects advice requests by asking the operator to set context (never "raise an issue/lawyer"). Legal-opinion floor absolute in both modes (DD-14 r2). Context = pointer(node_id/issue_id) resolved live per turn, sticky across the DD-40 window. **Brainstorm hand-off:** F11 card's "Brainstorm ↗" navigates here with issue+clauses as context, opening turn primed with the current recommendation; operator iterates ("more aggressive"/"shorter") and commits via the F11 apply path (DD-68). **Brainstorm reverted to ephemeral (DD-73/DD-77, SHIPPED 2026-06-25):** Brainstorm is a stateless ephemeral OVERLAY — the client holds the running transcript, the backend persists NOTHING per turn; on close Donna distils ONE compact summary (`{question, conclusion, fallbacks}`) into `brainstorm_summaries` (FK `issue_id`, one row per pass = history), surfaced in the issue's resolution view, then the raw transcript is discarded. Endpoints `POST …/donna/brainstorm` (stateless turn) · `POST …/donna/brainstorm/close` (distil→store or 204) · `GET /issues/{id}/brainstorm-summaries`; backend `services/donna/brainstorm.py` + `models/brainstorm.py` + 2 prompts + migration `0007` + DD-63 cascade extended; frontend right-anchored overlay reusing the chat/citation language. The DD-69 "conversation persists in the Donna thread" half is **withdrawn**; F10b's context-aware *advice* half stands. Adopted outcome still commits via the unchanged F11 apply path. Manual "Ask Donna about this clause" entry IN v1 (Lilly). DD-69. **OUT of v1:** live research (DD-38), F30 extraction from brainstorm turns, multi-clause/whole-deal advisory bundles, Donna-initiated context. **Backend:** `services/donna/advise.py` (`chat(contract_id, question, context)` — no anchor → delegates to F10 `qa.ask` preserved, advice request → soft `need_context`; anchor → live-resolved grounding reusing `build_clause_grounding`/`build_issue_focus` + F11/F08d framing, high tier, structured `{reply, mode∈explain|advise|draft|legal_referral|need_context, citations, draft_language}`, citation guard + id-scrub) + `prompts/donna_chat_advise_v1.txt` + `DonnaAskRequest.context{node_ids,issue_id}`; `mode`→3 schema `kind`s, **no migration**. **Frontend:** context chip (set via "Ask Donna about this clause" or Brainstorm ↗, `[×]` clears), mode-aware bubbles, **Brainstorm ↗** on the F11 card primes the chat with the current rec, draft `[Use this language]` reuses the DD-68 edited-confirm (issue) / node-edit (clause) apply paths — no new write. **Eval 7/7** (advise-with-anchor · draft-with-placeholder · elicit-before-recommend · **legal-opinion-walled-even-with-context** · no-context→need_context · F10 preserved · missing-benchmark honesty) + F10 regression 4/4; 433 backend tests; tsc 0. **Live screenshot-verified (engineering):** same advice question → no-context = deflect ("scoped to reading"), §12.1-in-context = advise ("recommendation") + cited + elicits the concern rather than inventing a stance. **Minor follow-ups (flagged, non-blocking):** Brainstorm's primed opening turn is client-only (not persisted to the backend thread — Donna re-derives context from issue_id on the next ask); the draft-apply effect couples to the rec async (silent no-op if generation errors). |
| F11 | Donna — issue-scoped recommendation + live research | P0 | 2 | **built** | Auto-generated when issue detail is opened; grounded in clause context + DD-31 resolution; live research invoked when issue involves market data (pricing, rates, thresholds); cites sources; proposes specific value/language; DD-38. **v1 scope (approved):** auto-on-open grounded recommendation as ONE object `{cited rationale + draft recommended landing + draft counter-language}`, framed on the DD-14 reasonableness spectrum + ask/settle/floor ladder; propose-vs-counter = same engine, different field; reuses Donna v1 infra + F05b grounding (no embeddings). **OUT of v1:** live research (DD-38), the Brainstorm overlay (slice 2), F28 flagging, F12 embeddings — a needed market figure → recommend the STRUCTURE + flag the missing benchmark, never invent a number. **Draft-vs-confirmed (DD-68):** the auto-recommendation is a DRAFT in `donna_recommendations`, never written to `issues.recommended_position`/`donna_counter_language` (F31 exports) until the operator confirms via [Use Donna's language]. **Backend BUILT + verified:** `services/donna/recommendations.py` (capable/high-tier structured rec, hallucinated-id guard + id scrub) + `recommendation_repo.py` (upsert-draft · get-by-issue · confirm = one-txn draft→issues.* copy + audit) + `prompts/donna_recommendation_v1.txt` + `models/recommendations.py` + `POST/GET /contracts/{cid}/issues/{iid}/recommendation` + `POST …/recommendation/confirm` (registered in main.py); migration `0003_donna_recommendations`; 20 tests; **grounding eval 4/4** (grounded+cited · propose · counter · honest missing-benchmark, no fabricated number — real Opus). **UI BUILT (STEP 3):** the recommendation card in the resolution view (DD-68) — auto-on-first-open (GET draft, else generate once; refresh regenerates), async loading, rationale + citation chips (reuse F10 resolver, jump+flash), recommended position + counter-language, honest market-gap note, and **[Use Donna's language] / [Edit] / [↻ Refresh] / ignore**. **[Edit]** lets the operator adjust Donna's language before applying it — confirmed via the **edited-confirm** path (DD-68 addendum): the confirm route takes an optional edited body, overwriting the draft inside the confirm txn before the `draft→issues.*` copy, so only operator-approved language reaches the F31 export. frontend tsc 0; backend 377 tests (mypy/ruff clean). |
...
| F27 | Version pointers + lineage view (where-are-we tracking) | P1 | 3 | **built** (badge + lineage view, engineering live-verified); received-pointer data (Mode B) + F21 diff still planned | 4 named snapshot pointers (DD-48); per-source diff baselines. **Built + live-verified (DD-75):** persistent lifecycle badge in the cockpit header AND every My Contracts card (`Sent to counterparty · v4` etc., colour-keyed); click-badge → Version-history drawer (Working copy pinned + v1…vN timeline with current-baseline tag + the 2 greyed `received` slots "arrives with revision import"); clicking a version opens it **read-only** ("Viewing v3 — read-only · Return to working copy"); per-card **Export** button after Edit. Frontend tsc 0; engineering screenshot-verified the full flow on the real app (card badge + cockpit badge + drawer + read-only open + return). **Backend (DD-75):** the lifecycle-badge resolver + lineage API are built — `services/lineage.py` (`derive_status` pure resolver · `derive_status_for_contracts` set-based card badge, no N+1 · `get_lineage` badge+timeline+reserved), `services/snapshot.py` read-only render adapter (`get_snapshot_tree`) + list helpers, `api/lineage.py` (`GET …/lineage`, `GET …/snapshots/{id}/tree`); set-based badge wired into the contract-list read so every card carries `badge`. Taxonomy `Working copy · Sent to counterparty · Sent to legal · Sent to counterparty & legal · Your move · Signed` + passive "edited since sent" marker; FIRST-MATCH-WINS derivation, ROW_NUMBER v-numbers, marker vs most-recent `last_shared_with_*` snapshot (not `last_export_at`). **Pending:** the Phase-2 received-pointer data (Mode B) + the F21 version diff (lineage v1 = read-only open + 2 greyed `received` slots; diff/restore out). **Boundary action = "Mark as sent → Counterparty / Legal / Both" (DD-71) — SHIPPED:** the deliberate step that cuts the snapshot (current working copy), advances `last_shared_with_X` (one snapshot may carry both pointers), and mints the next v-number — built in the **cockpit AND each My Contracts card**; stamps current state with a **non-blocking drift warning** if edited since the last export (DD-72: `contracts.last_export_at` + `node.updated_at` probe; mark-anyway / re-export). Export no longer sets pointers (DD-71). Backend `services/mark_sent.py` + `api/mark_sent.py` (`POST …/mark-sent {recipient, acknowledge_drift?}` → `{marked, drift, version, pointers, snapshot_id, last_export_at}`); frontend `markSent` + cockpit "Mark as sent ▾" control + per-card panel; 11 tests. **NOW BUILT (DD-75, engineering live-verified):** the lineage **view**, the persistent cockpit + My-Contracts-card lifecycle **badge** (replaces the old transient toast), and the "edited since sent" marker. **STILL PLANNED:** the 2 `received` pointer states (Mode B / F03b populates them) + F21 version diff. **Lineage = monotonic v-numbers over snapshots (DD-70):** every boundary event — a send (→ counterparty/legal) or a receive (← counterparty/legal) — mints the next integer on one timeline, direction-tagged (`v3 → sent to counterparty`, `v4 ← received from counterparty`); the **working copy is never numbered or locked** (numbers attach only to frozen snapshots). **Unsent-edits marker (DD-70):** when the working copy diverges from `last_shared_with_counterparty` (unassigned `node_versions` since that pointer), a single consolidated **passive "edited since last send"** indicator shows beside the badge — non-blocking, no lock/confirm/nudge; derived, no schema change. **Status taxonomy (the home-card "where are we" badge, derived not manually set — tracks the last boundary event with the other side, NOT local editing, DD-70):** `Your move` (received from counterparty/legal, untouched — reactive, floats up) → operator engages it → `In flight` (actively being worked: a fresh draft OR an engaged *inbound* revision; self-paced) → operator sends → `Sent to counterparty` / `Sent to legal` (waiting on them; **post-send local edits keep this badge + raise the unsent-edits marker — they do NOT flip to `In flight`**) → returns → `Your move` … → `Signed`. The home card shows this badge + open-issue count (red when >1) + last activity. Full pointer-states need snapshots (Phase 3) + revision import (Phase 2); the Phase-1 home shows `In flight` + open-issues + recency, richer states fill in by phase |
...
All design decision records (DD-01 … DD-82) live in **[`DESIGN_DECISIONS.md`](DESIGN_DECISIONS.md)** — see its index for the full list. Inline `DD-NN` references throughout this spec resolve there.
...
- **Each card:** a **status badge** (the where-are-we taxonomy, F27/DD-48: `Your move` / `In flight` / `Sent to counterparty` / `Sent to legal` / `Signed` — color-coded: amber = your move, green = signed, neutral = in flight, muted = waiting; tracks the last boundary event with the other side, not local edits — DD-70) · an **"edited since last send" marker** when the working copy has diverged from the last shared snapshot (passive, non-blocking — DD-70) · contract name + client·deal · **open-issue count (red when > 1, else grey)** · last activity.
...
**Clean copy (.docx) — F15b.** Regenerates the full contract from the DB through the style config — current state, no markup. **A pure download** (DD-71): no recipient selector, no snapshot, no pointer, no lineage effect — exporting is just "give me the file." The operator sends it herself (the app has no transport), then records it via **Mark as sent** (below). The foundation the other two exports build on.

**Mark as sent — F27/DD-71.** The boundary event, decoupled from export because donna.ai can't actually send. A deliberate **`Mark as sent → Counterparty | Legal`** action, in the **cockpit and on each My Contracts card**, that: cuts an immutable snapshot of the **current working copy**, advances that party's `last_shared_with_X` pointer (the diff baseline for their next inbound revision), flips the lifecycle badge to `Sent to counterparty` / `Sent to legal`, and mints the next version number in the lineage (DD-70). One snapshot may carry both pointers if sent to both (DD-48). **Drift guard (non-blocking):** if the working copy is unchanged since the last export, mark silently; if she has edited since exporting, a one-click-through heads-up — *"You've edited since your last export (vN, date). Marking now records your CURRENT version as sent. [Mark anyway] [Re-export]"* — never a gate. (Supersedes the old single "Mark as sent to counterparty" checkbox and the DD-61 send-on-export model.)
...
- **Requires a baseline:** a brand-new contract has no snapshot to diff against, so Redline is **disabled until the first Mark as sent (Counterparty / Legal) cuts a snapshot** (DD-71 — exports no longer cut snapshots), with the hint *"No baseline yet — export a clean copy, send it, and Mark as sent first; from the next round you can send redlines."* This matches the core workflow: round 1 = clean copy + mark sent, every round after = redline + mark sent.
...
- **Brainstorm with Donna ↗** — opens a chat overlay pre-loaded with the clause + issue description. Back-and-forth exploration ("what if we propose a tiered rate?", "what's the floor we should accept?") without leaving context. The conversation is **ephemeral** — not persisted; it opens fresh each time. If the brainstorm produces language the operator accepts, that committed outcome applies to the issue through the standard apply path (adopted language lands on the issue, node version + audit entry written, decision logged per DD-29). The exploratory chat and any rejected intermediate proposals are discarded (DD-42) — but **on close Donna distills a compact, readable summary** (the question explored, the position concluded, the key fallbacks considered) and **stores it on the issue** so the reasoning survives without the transcript (DD-73). This is separate from DD-55's silent cross-deal pattern extraction; both run on close. Donna's grounding still reads only the committed ledger + clause text, never the summary or transcript.
