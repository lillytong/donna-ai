...
| F30 | Negotiation insight distillation | P1 | 2+ | **read-half scaffold built; WRITER pending** | **Trigger re-anchored to ISSUE-CLOSE (Lilly-approved 2026-06-25; not "brainstorm close", which has no built lifecycle) — distil from the committed issue ledger, never the transcript.** **DONE + committed (dormant, no behavior change):** `negotiation_patterns` table (operator-global, scoped by `subject_ref`; migration `0006`), `services/donna/distillation.py` read fns, `grounding.build_pattern_grounding`, and the F11 retrieval hook in `recommendations.py` (injects a learned-pattern block; table is empty so zero effect until the writer exists). **PENDING (next session = the one remaining build):** the issue-close async writer (failure-isolated → extract→merge-first dedup→store), consolidation/prune completion, **DD-76** (reconciles DD-55↔DD-73), tests + eval, then flip this row to built. Engineering ADRs settled: extraction at medium/Sonnet tier; LLM-judge merge over the tiny pre-filtered candidate set (no embeddings). After each issue closes, Donna runs an extraction pass and synthesizes 0–N compact pattern records — not the raw conversation. Merge-first: checks existing patterns before creating; updates evidence count + refines wording on a match. Consolidation pass triggered after deal close or N new patterns added: redundant patterns merged, low-confidence patterns past TTL (3 deals unreinforced) pruned, contradictions surfaced as flags not silent overwrites. Patterns retrieved selectively alongside tiered context when Donna opens an issue. Converges to ~100–200 records across all subjects; never grows unbounded. DD-55 |
...
All design decision records (DD-01 … DD-74) live in **[`DESIGN_DECISIONS.md`](DESIGN_DECISIONS.md)** — see its index for the full list. Inline `DD-NN` references throughout this spec resolve there.
