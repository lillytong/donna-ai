...
| F03e | Inline enumerator splitting — `(a)(b)(c)` / `(i)(ii)(iii)` → child nodes | P1 | 1 | **built** | **Greenlit by Lilly 2026-06-25, reverses the prior won't-fix-v1** (§6). At parse time, an inline enumerator run inside one paragraph splits into ordered children of its lead-in clause. **Acceptance (met):** a paragraph `lead-in: (a) X (b) Y` parses to **1 parent + 2 children** — the lead-in text is the parent body, each `(a)`/`(b)` an ordered child, **child order preserved**. **Edge rules (§6):** defined-term definitions (`"Term" means (i)… (ii)…`) are **never split** (permanent carve-out); **flat-only in v1** (nested `(a)…(i)…(b)` not recursed); child body **retains its own `(a)`/`(i)` marker** as native text (Donna doesn't re-derive alpha/roman); **new imports only** — committed trees are not retroactively re-split. **Round-trip de-risk gate HELD (2026-06-25):** reassembly oracle byte-identical on the synthetic always-runs fixture AND the real demo contract; split is content-lossless by construction (split only at whitespace-bounded markers on normalised text → join reproduces source exactly), so a mis-detection can only add an operator-correctable node, never corrupt content. Built in `services/import_/inline_split.py` (`split_inline_enumerators`), wired into the import pipeline after role stamping. Full suite 373 green, mypy/ruff clean. See §6 inline-enumeration note. **⚠ CORRECTION (2026-06-30, DD-99): the "wired into the import pipeline" claim is FALSE — `split_inline_enumerators` is referenced only by its own tests, never imported into `pipeline.py`/anywhere in `backend/`, so the inline split never runs on a real import. Not wired during F03f build (export-paragraph round-trip implications); needs separate scoping (wire + extend the export round-trip oracle, or formally re-scope). Inline `(a)(b)(c)` are currently NOT split on import.** |
| F03f | Block enumerated items — `(a)(b)(c)` / `(A)(B)(C)` / `(i)(ii)(iii)` blocks keep marker, not decimal-renumbered | P1 | 1 | **built** | **✅ ACCEPTED by Lilly 2026-06-30** after the full multi-layer fix (DD-100 Parts 1–6), verified on the real JVA via import→export→Word. The earlier "FAILED acceptance" is resolved: Part 1 closed the API-boundary `enumerator_format` leaks + frontend `deriveNumbers` markers + `(1)(2)(3)` paren-detection (migration 0021); the parser nesting was rebuilt on a per-scheme **outline stack** (depth = absolute-`ilvl` capped to the right-spine, leaf-aware) so `1.2.1`'s `(a)…(r)` nest under 1.2.1, a defined term's `(a)…(e)` nest under it as `1.1.72(a)`, an `(a)` carrying an `(A)(B)` sub-list resumes as `(b)(c)` (no double `(a)`), and the backbone never advances onto enumerated items (next term → `1.1.73`); export emits per-run abstractNums (each list restarts at `(a)`) with depth-based indent, grouping runs by parent/sibling; the import tree re-derives markers on re-indent. Backend 808 pass, frontend 14 pass, zero parentless nodes, round-trip preserved. The 1.2.1 caveat below is **resolved**. **Built 2026-06-30 (DD-98 amended / DD-99 — Option B, auto-renumber).** `(a)(b)(c)`/`(A)(B)(C)`/`(i)(ii)(iii)` items are kept as per-item nodes whose marker is **derived from position & auto-renumbers** (delete `(a)`→`(b)` becomes `(a)`), NOT decimal-renumbered. Import resolves `<w:numPr>`→numbering.xml, captures `numFmt` in **`nodes.enumerator_format`** (migration 0020), strips the marker from body; the glyph is derived per ordinal among consecutive enumerated siblings (**restart-per-list-run**, not per-numId — one numId is reused across 36 lists); export emits **native Word auto-numbering** so Word renumbers too. **Verified on the real JVA:** 5.4.1→`(a)(b)(c)(d)` (no decimal inflation); delete-renumbers confirmed (18-item list, delete first→all shift); 176 enumerated resolved (113 lowerLetter/33 upperLetter/24 lowerRoman/6 literal recitals — the earlier "405" was an overcount, nothing literal missed). Gate 794 pass/1 skip, mypy+ruff clean; fixed 2 latent bugs (`node_edit`/`node_create` SELECTs omitted the new column → would crash every edit/create post-migration). Round-trip green across all 4 formats. **Open caveat (Lilly to judge at acceptance):** 1.2.1's `(a)…(r)` list attaches under 1.2 (sibling of 1.2.1) rather than nested beneath 1.2.1 — likely pre-existing parser nesting; acceptance core (markers, no decimal) holds. New imports only. **F03e wiring bug (separate):** `split_inline_enumerators` is test-only, never wired into the pipeline — inline split doesn't run on real imports; needs its own scoping (DEV_TODO). See §6 block-enumeration note; DD-98/DD-99 |
...
| F38 | Generate redlines — full-contract first-pass redline on new import | P1 | 2+ | **scoped** (v1 MUST/LATER locked, DD-97; not built) | Operator-triggered **"Generate redlines" button** in the cockpit (Mode-A first imports only) → Donna drafts recommended counter-language on **every operative clause**, riding the **DD-93 recommendation engine** (F32 + F37 + F36 + F34 + F35 + F30) — F38 is that engine with the input unit set to *every clause* rather than a counterparty change cluster (F03c) or an opened issue (F11), not new machinery. Opening stance **parameterized by `contracts.origin`** — three different jobs (us → **protective in-house-counsel review** for the non-lawyer operator, catch blind spots · our_legal → **temper over-reach** likely to draw counterparty pushback, propose still-favorable-or-balanced · counterparty → **full scrutiny**; DD-84/DD-97, DECIDED by Lilly 2026-06-29). Each clause card carries a **materiality signal** (would-accept / minor / substantive, reusing the F34 significance split) so the operator triages, not wades. Rides the existing recommendation-card surface (DD-93/F11) + F35 citations; all F11 guardrails hold — cite-or-flag, never invents a figure (bracketed placeholder), "get a lawyer" on enforceability, never auto-applied, never auto-exported, never auto-seeds issues (§2.4 / DD-14 / DD-50). **Distinct from F28** (triage: flag problems) — F38 is position-setting (draft on every clause). **Operator-triggered, never auto-at-import** (DD-96). **v1 boundary + open taste calls (stance calibration · card-stream vs dedicated whole-doc view · F11 coexistence) in DD-97 + PM_TODO.** Prereq MET: `contracts.origin` captured at import (F04, built; DD-84). DD-96/DD-97 |
...
**Block enumeration note (DD-98, F03f — greenlit by Lilly 2026-06-30).**
`(a)(b)(c)…` / `(A)(B)(C)…` / `(i)(ii)(iii)…` enumerated items that appear as **separate blocks** directly under an operative clause (each item already its own paragraph — the dominant structural pattern, distinct from the inline run F03e splits) are parsed so that:

- **Each item is its own ordered child node** of the lead-in clause — per-item addressable (issue / redline a single `(a)`). NOT collapsed into one opaque list block.
- **The `(a)`/`(A)`/`(i)` marker is DERIVED from list position and AUTO-RENUMBERS** (amended 2026-06-30) — the identical model as decimal clause numbers (DD-02, position-derived, never stored). Delete `(a)X` from `(a)X (b)Y (c)Z` → `(a)Y (b)Z`. The list **format** (lowerLetter/upperLetter/lowerRoman/upperRoman) is captured from the source `numbering.xml` (declared there, no inference) and **stored** (`nodes.enumerator_format`); the glyph is derived per position + restart-per-parent. Exported as native Word auto-numbering (Word renumbers too).
- **Enumerated items are NOT decimal-renumbered.** Addressed by **parent clause + marker** — "clause 1.2.1(b)", the legal-drafting convention — never "1.2.1.2". `derive_numbers` gives these the derived alpha/roman marker, not a decimal (extends the DD-02/DD-54 clause-only rule).
- **Defined-terms carve-out + flat-only + new-imports-only** all carry over from F03e unchanged.
- *(Eng note, DD-98/DD-99: import resolves `<w:numPr>`→numbering.xml to detect alpha/roman levels + capture format; marker stripped from body and derived on render; export emits native Word auto-numbering. Round-trip stable — numPr written on export, re-read on import, marker derived both sides.)*
