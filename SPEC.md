...
| F04 | Import-review UI — first import (verify/correct parse before commit) | P0 | 0 | **built** | First-import review UI (Next.js two-panel tree+source): verify/correct the parse before commit — confidence flags, triage counter, commit gating, uncertain nodes highlighted, on real parses (never trust parse blindly). Role-region rendering (DD-54: front-/back-matter as labeled regions, drafting-notes flagged, non-clause rows unnumbered) + AI back-matter categorization (DD-56/58: known appendix-title designators detected deterministically as dividers, a Haiku whole-region pass categorizes the rest and can promote unseen designators). Style detected at import (DD-37). **All correction ops complete** — inline level/type/role edit (role re-buckets the node live), multi-select bulk shift, Move ↑/↓ + reparent (commit keyed to operator sequence), split, merge, content-type (Heading/Body/Table) corrections — each round-trips to commit with no-data-loss guards. Only a cosmetic visual-identity pass remains (not a correction gap). **Post-commit destination:** on commit success, redirect to the cockpit (`/contracts/[id]`). **`contracts.origin` capture (pending, DD-84):** the import flow must include a step where the operator declares who authored this contract (`us / our_legal / counterparty`); this field drives Donna's opening stance for F28 and closes the §6 ↔ schema.sql gap flagged in DD-84. DD-37/54/56/58/84. Build detail in git. |
...
| F38 | Generate redlines — full-contract first-pass redline on new import | P1 | TBD (needs sequencing; prerequisite: F04 `contracts.origin` capture) | planned | When the operator imports a new contract (Mode A — first version, not a counterparty revision), a **"Generate redlines" button** in the cockpit triggers Donna to produce recommended counter-language for **every clause** in the contract. Donna's opening stance is **parameterized by `contracts.origin`** (us / our\_legal / counterparty — DD-84): the posture shifts depending on whether we, our legal counsel, or the counterparty authored the baseline (exact stance per origin value is an open product question, PM_TODO). Output surface TBD — likely reuses or extends the existing recommendation-card UI (F11/DD-93), but the full-contract scope and triggered-not-lazy delivery may warrant a distinct surface. **Distinct from F28** (issue detection): F28 flags problems clause by clause; F38 is a full-draft redline — drafted counter-language on every clause, not just flagged ones. Operator-triggered only — button click, not auto-generated at import. Prerequisite: `contracts.origin` must be captured at import (F04 gap — open in DEV_TODO; DD-84). DD-96 |
...
All design decision records (DD-01 … DD-95) live in **[`DESIGN_DECISIONS.md`](DESIGN_DECISIONS.md)** — see its index for the full list. Inline `DD-NN` references throughout this spec resolve there.
